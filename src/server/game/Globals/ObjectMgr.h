...
// Neo string ranges
#define MIN_NEO_STRING_ID           1                    // 'neo_string'
...
// Neo Trainer Reference start range
#define NEO_TRAINER_START_REF      200000
...
typedef UNORDERED_MAP<int32,NeoStringLocale> NeoStringLocaleMap;
...
class ObjectMgr
{
    ...
    public:
        ...
        bool LoadNeoStrings(char const* table, int32 min_value, int32 max_value);
        bool LoadNeoStrings() { return LoadNeoStrings("neo_string",MIN_NEO_STRING_ID,MAX_NEO_STRING_ID); }
        ...
        const char *GetNeoString(int32 entry, LocaleConstant locale_idx) const;
...
// scripting access functions
 bool LoadNeoStrings(char const* table,int32 start_value = MAX_CREATURE_AI_TEXT_STRING_ID, int32 end_value = std::numeric_limits<int32>::min());
 uint32 GetAreaTriggerScriptId(uint32 trigger_id);
 uint32 GetScriptId(const char *name);
