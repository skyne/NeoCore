...
{
    CellPair p = Neo::ComputeCellPair(obj->GetPositionX(), obj->GetPositionY());
...
{
    CellPair pair = Neo::ComputeCellPair(x, y);
    Cell cell(pair);
    EnsureGridLoaded(cell);
}
...
void Map::Update(const uint32 &t_diff)
{
    ...
    Neo::ObjectUpdater updater(t_diff);
    // for creature
    TypeContainerVisitor<Neo::ObjectUpdater, GridTypeMapContainer  > grid_object_update(updater);
    // for pets
    TypeContainerVisitor<Neo::ObjectUpdater, WorldTypeMapContainer > world_object_update(updater);
    ...
    for (m_mapRefIter = m_mapRefManager.begin(); m_mapRefIter != m_mapRefManager.end(); ++m_mapRefIter)
    {
        ...
        CellPair standing_cell(Neo::ComputeCellPair(plr->GetPositionX(), plr->GetPositionY()));
    ...
    if (!m_activeNonPlayers.empty())
    {
        for (m_activeNonPlayersIter = m_activeNonPlayers.begin(); m_activeNonPlayersIter != m_activeNonPlayers.end();)
        {
            ...
            CellPair standing_cell(Neo::ComputeCellPair(obj->GetPositionX(), obj->GetPositionY()));
    ...
    for (GridRefManager<NGridType>::iterator i = GridRefManager<NGridType>::begin(); i != GridRefManager<NGridType>::end(); ++i)
    {
        ...
        for (uint32 x = cell_min.x_coord; x < cell_max.x_coord; ++x)
        {
            for (uint32 y = cell_min.y_coord; y < cell_max.y_coord; ++y)
            {
                ...
                Neo::DelayedUnitRelocation cell_relocation(cell, pair, *this, GetVisibilityDistance());
                TypeContainerVisitor<Neo::DelayedUnitRelocation, GridTypeMapContainer  > grid_object_relocation(cell_relocation);
                TypeContainerVisitor<Neo::DelayedUnitRelocation, WorldTypeMapContainer > world_object_relocation(cell_relocation);
...
Map::PlayerRelocation(Player *player, float x, float y, float z, float orientation)
{
    ...
    CellPair old_val = Neo::ComputeCellPair(player->GetPositionX(), player->GetPositionY());
    CellPair new_val = Neo::ComputeCellPair(x, y);
...
void Map::MoveAllCreaturesInMoveList()
{
    while (!i_creaturesToMove.empty())
    {
        ...
        // calculate cells
        CellPair new_val = Neo::ComputeCellPair(cm.x, cm.y);
        Cell new_cell(new_val);
...
bool Map::CreatureRespawnRelocation(Creature *c)
{
    ...
    CellPair resp_val = Neo::ComputeCellPair(resp_x, resp_y);
...
bool Map::CheckGridIntegrity(Creature* c, bool moved) const
{
    ...
    CellPair xy_val = Neo::ComputeCellPair(c->GetPositionX(), c->GetPositionY());
    Cell xy_cell(xy_val);
    if (xy_cell != cur_cell)
    {
...
void Map::UpdateObjectVisibility(WorldObject* obj, Cell cell, CellPair cellpair)
{
    ...
    cell.SetNoCreate();
    Neo::VisibleChangesNotifier notifier(*obj);
    TypeContainerVisitor<Neo::VisibleChangesNotifier, WorldTypeMapContainer > player_notifier(notifier);
    cell.Visit(cellpair, player_notifier, *this, *obj, GetVisibilityDistance());
}

void Map::UpdateObjectsVisibilityFor(Player* player, Cell cell, CellPair cellpair)
{
    Neo::VisibleNotifier notifier(*player);

    cell.data.Part.reserved = ALL_DISTRICT;
    cell.SetNoCreate();
    TypeContainerVisitor<Neo::VisibleNotifier, WorldTypeMapContainer > world_notifier(notifier);
    TypeContainerVisitor<Neo::VisibleNotifier, GridTypeMapContainer  > grid_notifier(notifier);
...
bool Map::ActiveObjectsNearGrid(uint32 x, uint32 y) const
{
    ...
    for (MapRefManager::const_iterator iter = m_mapRefManager.begin(); iter != m_mapRefManager.end(); ++iter)
    {
        ...
        CellPair p = Neo::ComputeCellPair(plr->GetPositionX(), plr->GetPositionY());
...
void Map::AddToActive(Creature* c)
{
    ...
    if (!c->isPet() && c->GetDBTableGUIDLow())
    {
        ...
        GridPair p = Neo::ComputeGridPair(x, y);
        if (getNGrid(p.x_coord, p.y_coord))
            getNGrid(p.x_coord, p.y_coord)->incUnloadActiveLock();
        else
        {
            GridPair p2 = Neo::ComputeGridPair(c->GetPositionX(), c->GetPositionY());
...
inline GameObject* Map::_FindGameObject(WorldObject* pSearchObject, uint32 guid) const
{
    ...
    CellPair p(Neo::ComputeCellPair(pSearchObject->GetPositionX(), pSearchObject->GetPositionY()));
    Cell cell(p);
    cell.data.Part.reserved = ALL_DISTRICT;

    Neo::GameObjectWithDbGUIDCheck goCheck(*pSearchObject, guid);
    Neo::GameObjectSearcher<Neo::GameObjectWithDbGUIDCheck> checker(pSearchObject, pGameObject, goCheck);

    TypeContainerVisitor<Neo::GameObjectSearcher<Neo::GameObjectWithDbGUIDCheck>, GridTypeMapContainer > objectChecker(checker);
        ...
        switch (step.script->command)
        {
            case SCRIPT_COMMAND_TALK:
                ...
                if (step.script->datalong2 & 0x1)
                {
                    if (Player *pSource = _GetScriptPlayerSourceOrTarget(source, target, step.script))
                    {
                        ...
                        const std::string text(sObjectMgr.GetNeoString(step.script->dataint, loc_idx));
            ...
            case SCRIPT_COMMAND_CALLSCRIPT_TO_UNIT:
            {
                ...
                if (source) //using grid searcher
                {
                    ...
                    CellPair p(Neo::ComputeCellPair(wSource->GetPositionX(), wSource->GetPositionY()));
                    Cell cell(p);
                    cell.data.Part.reserved = ALL_DISTRICT;

                    Neo::CreatureWithDbGUIDCheck target_check(wSource, step.script->datalong);
                    Neo::CreatureSearcher<Neo::CreatureWithDbGUIDCheck> checker(wSource, cTarget, target_check);
