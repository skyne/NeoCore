...
bool ChatHandler::HandleGameObjectAddCommand(const char* args)
{
    ...
    sLog.outDebug(GetNeoString(LANG_GAMEOBJECT_CURRENT), gInfo->name, db_lowGUID, x, y, z, o);
...
bool ChatHandler::HandlePInfoCommand(const char* args)
{
    ...
    std::string email = GetNeoString(LANG_ERROR);
    ...
    PSendSysMessage(LANG_PINFO_ACCOUNT, (target?"":GetNeoString(LANG_OFFLINE)), nameLink.c_str(), GUID_LOPART(target_guid), username.c_str(), accId, email.c_str(), security, last_ip.c_str(), last_login.c_str(), latency);
    ...
    for (uint32 id = 0; id < sCharTitlesStore.GetNumRows(); id++)
    {
        ...
        if (titleInfo)
        {
            ...
            if (!name.empty())
            {
                ...
                char const* knownStr = target && target->HasTitle(titleInfo) ? GetNeoString(LANG_KNOWN) : "";

                char const* activeStr = target && target->GetUInt32Value(PLAYER_CHOSEN_TITLE) == titleInfo->bit_index
                    ? GetNeoString(LANG_ACTIVE)
                    : "";
...
bool ChatHandler::HandleCharacterTitlesCommand(const char* args)
{
    ...
    LocaleConstant loc = GetSessionDbcLocale();
    char const* targetName = target->GetName();
    char const* knownStr = GetNeoString(LANG_KNOWN);
