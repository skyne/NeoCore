...
bool ChatHandler::HandleAnnounceCommand(const char* args)
{
    ...
    char buff[2048];
    sprintf(buff, GetNeoString(LANG_SYSTEMMESSAGE), args);
...
bool ChatHandler::HandleGPSCommand(const char* args)
{
    ...
    sLog.outDebug("Player %s GPS call for %s '%s' (%s: %u):",
        m_session ? GetNameLink().c_str() : GetNeoString(LANG_CONSOLE_COMMAND),
...
bool ChatHandler::HandleSummonCommand(const char* args)
{
    ...
    else
    {
        ...
        PSendSysMessage(LANG_SUMMONING, nameLink.c_str(),GetNeoString(LANG_OFFLINE));
...
/*    MapEntry const * me = sMapStore.LookupEntry(tele->mapId);
    ...
    else
    {
        ...
        PSendSysMessage(LANG_TELEPORTING_TO, nameLink.c_str(), GetNeoString(LANG_OFFLINE), tele->name.c_str());
