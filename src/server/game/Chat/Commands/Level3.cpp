...
bool ChatHandler::HandleCooldownCommand(const char *args)
{
    ...
    else
    {
        ...
        {
            PSendSysMessage(LANG_UNKNOWN_SPELL, target == m_session->GetPlayer() ? GetNeoString(LANG_YOU) : tNameLink.c_str());
            SetSentErrorMessage(true);
            return false;
        }

        target->RemoveSpellCooldown(spell_id,true);
        PSendSysMessage(LANG_REMOVE_COOLDOWN, spell_id, target == m_session->GetPlayer() ? GetNeoString(LANG_YOU) : tNameLink.c_str());
   }
    return true;
}
        ...
        if (skillInfo)
        {
            ...
            if (!name.empty())
            {
                ...
                if (target && target->HasSkill(id))
                {
                    ...
                    char const* valFormat = GetNeoString(LANG_SKILL_VALUES);
        ...
        if (factionEntry)
        {
            ...
            if (!name.empty())
            {
                ...
                if (repState)                               // and then target != NULL also
                {
                    ...
                    std::string rankName = GetNeoString(index);

                    ss << " " << rankName << "|h|r (" << target->GetReputationMgr().GetReputation(factionEntry) << ")";

                    if (repState->Flags & FACTION_FLAG_VISIBLE)
                        ss << GetNeoString(LANG_FACTION_VISIBLE);
                    if (repState->Flags & FACTION_FLAG_AT_WAR)
                        ss << GetNeoString(LANG_FACTION_ATWAR);
                    if (repState->Flags & FACTION_FLAG_PEACE_FORCED)
                        ss << GetNeoString(LANG_FACTION_PEACE_FORCED);
                    if (repState->Flags & FACTION_FLAG_HIDDEN)
                        ss << GetNeoString(LANG_FACTION_HIDDEN);
                    if (repState->Flags & FACTION_FLAG_INVISIBLE_FORCED)
                        ss << GetNeoString(LANG_FACTION_INVISIBLE_FORCED);
                    if (repState->Flags & FACTION_FLAG_INACTIVE)
                        ss << GetNeoString(LANG_FACTION_INACTIVE);
                }
                else
                    ss << GetNeoString(LANG_FACTION_NOREPUTATION);
        ...
        if (MapInfo)
        {
            ...
            if (loc < MAX_LOCALE)
            {
                ...
                if (MapInfo->IsContinent())
                    ss << GetNeoString(LANG_CONTINENT);

                switch(MapInfo->map_type)
                {
                    case MAP_INSTANCE:      ss << GetNeoString(LANG_INSTANCE);      break;
                    case MAP_BATTLEGROUND:  ss << GetNeoString(LANG_BATTLEGROUND);  break;
                    case MAP_ARENA:         ss << GetNeoString(LANG_ARENA);         break;
                }

                if (MapInfo->IsRaid())
                    ss << GetNeoString(LANG_RAID);

                if (MapInfo->SupportsHeroicMode())
                    ss << GetNeoString(LANG_HEROIC);
                ...
                if (MapInfo->IsMountAllowed())
                    ss << GetNeoString(LANG_MOUNTABLE);
...
bool ChatHandler::HandleBanInfoCharacterCommand(const char *args)
{
    ...
    do
    {
        ...
        std::string bantime = permanent ? GetNeoString(LANG_BANINFO_INFINITE) : secsToTimeString(result->GetUInt64(1), true);
        PSendSysMessage(LANG_BANINFO_HISTORYENTRY,
            result->GetCString(0), bantime.c_str(), active ? GetNeoString(LANG_BANINFO_YES) : GetNeoString(LANG_BANINFO_NO), result->GetCString(4), result->GetCString(5));
    } while (result->NextRow());
...
bool ChatHandler::HandleRespawnCommand(const char* /*args*/)
{
    ...
    Neo::RespawnDo u_do;
    Neo::WorldObjectWorker<Neo::RespawnDo> worker(pl,u_do);
