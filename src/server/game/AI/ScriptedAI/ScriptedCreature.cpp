...
{
    Unit* pUnit = NULL;
    Neo::MostHPMissingInRange u_check(me, fRange, uiMinHPDiff);
...
std::list<Creature*> ScriptedAI::DoFindFriendlyCC(float fRange)
{
    ...
    Neo::FriendlyCCedInRange u_check(me, fRange);
    Neo::CreatureListSearcher<Neo::FriendlyCCedInRange> searcher(me, pList, u_check);
    me->VisitNearbyObject(fRange, searcher);
    return pList;
}
...
{
    std::list<Creature*> pList;
    Neo::FriendlyMissingBuffInRange u_check(me, fRange, uiSpellid);
    Neo::CreatureListSearcher<Neo::FriendlyMissingBuffInRange> searcher(me, pList, u_check);
    me->VisitNearbyObject(fRange, searcher);
    return pList;
}
...
Player* ScriptedAI::GetPlayerAtMinimumRange(float fMinimumRange)
{
    ...
    CellPair pair(Neo::ComputeCellPair(me->GetPositionX(), me->GetPositionY()));
    Cell cell(pair);
    cell.data.Part.reserved = ALL_DISTRICT;
    cell.SetNoCreate();

    Neo::PlayerAtMinimumRangeAway check(me, fMinimumRange);
    Neo::PlayerSearcher<Neo::PlayerAtMinimumRangeAway> searcher(me, pPlayer, check);
