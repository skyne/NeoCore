...
namespace Neo
{
    ...
    class Battleground2ChatBuilder
    {
        public:
            ...
            {
                char const* text = sObjectMgr.GetNeoString(i_textId,loc_idx);
                char const* arg1str = i_arg1 ? sObjectMgr.GetNeoString(i_arg1,loc_idx) : "";
                char const* arg2str = i_arg2 ? sObjectMgr.GetNeoString(i_arg2,loc_idx) : "";
...
uint32 Battleground::GetBonusHonorFromKill(uint32 kills) const
{
    ...
    return Neo::Honor::hk_honor_at_level(maxLevel, kills);
}
...
{
    Neo::BattlegroundChatBuilder bg_builder(type, entry, source);
    Neo::LocalizedPacketDo<Neo::BattlegroundChatBuilder> bg_do(bg_builder);
...
void Battleground::PSendMessageToAll(int32 entry, ChatMsg type, Player const* source, ...)
{
    ...
    Neo::BattlegroundChatBuilder bg_builder(type, entry, source, &ap);
    Neo::LocalizedPacketDo<Neo::BattlegroundChatBuilder> bg_do(bg_builder);
    BroadcastWorker(bg_do);
...
{
    const char *format = sObjectMgr.GetNeoStringForDBCLocale(entry);
    va_list ap;
    char str [1024];
    va_start(ap, entry);
...
{
    Neo::Battleground2ChatBuilder bg_builder(type, entry, source, arg1, arg2);
    Neo::LocalizedPacketDo<Neo::Battleground2ChatBuilder> bg_do(bg_builder);
    BroadcastWorker(bg_do);
}
...
//to be removed
const char *Battleground::GetNeoString(int32 entry)
{
    // FIXME: now we have different DBC locales and need localized message for each target client
    return sObjectMgr.GetNeoStringForDBCLocale(entry);
}
...
void Battleground::RewardXPAtKill(Player* plr, Player* victim)
{
    ...
    if (Group *pGroup = plr->GetGroup())//should be always in a raid group while in any bg
    {
        for (GroupReference *itr = pGroup->GetFirstMember(); itr != NULL; itr = itr->next())
        {
            ...
            uint32 gray_level = Neo::XP::GetGrayLevel(member->getLevel());
        ...
        {
            xp = !not_gray_member_with_max_level ? 0 : Neo::XP::Gain(not_gray_member_with_max_level, victim);
