...
void Player::Regenerate(Powers power)
{
    ...
    switch (power)
    {
        case POWER_MANA:
        {
            ...
            if (recentCast) // Neo Updates Mana in intervals of 2s, which is correct
...
void Player::UpdateCombatSkills(Unit *pVictim, WeaponAttackType attType, bool defence)
{
    ...
    uint8 greylevel = Neo::XP::GetGrayLevel(plevel);
...
void Player::SendActionButtons(uint32 state) const
{
    ...
    /*
        state can be 0, 1, 2
        0 - Looks to be sent when initial action buttons get sent, however on Neo we use 1 since 0 had some difficulties
        1 - Used in any SMSG_ACTION_BUTTONS packet with button data on Neo. Only used after spec swaps on retail.
...
void Player::SendMessageToSet(WorldPacket *data, bool self)
{
    ...
    Neo::MessageDistDeliverer notifier(this, data, GetMap()->GetVisibilityDistance());
...
void Player::SendMessageToSetInRange(WorldPacket *data, float dist, bool self)
{
    ...
    Neo::MessageDistDeliverer notifier(this, data, dist);
...
void Player::SendMessageToSetInRange(WorldPacket *data, float dist, bool self, bool own_team_only)
{
    ...
    Neo::MessageDistDeliverer notifier(this, data, dist, own_team_only);
    VisitNearbyWorldObject(dist, notifier);
}
...
void Player::SendMessageToSet(WorldPacket *data, Player const* skipped_rcvr)
{
    ...
    // we use World::GetMaxVisibleDistance() because i cannot see why not use a distance
    // update: replaced by GetMap()->GetVisibilityDistance()
    Neo::MessageDistDeliverer notifier(this, data, GetMap()->GetVisibilityDistance(), false, skipped_rcvr);
    VisitNearbyWorldObject(GetMap()->GetVisibilityDistance(), notifier);
}
...
int32 Player::CalculateReputationGain(uint32 creatureOrQuestLevel, int32 rep, int32 faction, bool for_quest, bool noQuestBonus)
{
    ...
    if (rate != 1.0f && creatureOrQuestLevel <= Neo::XP::GetGrayLevel(getLevel()))
        percent *= rate;
...
bool Player::RewardHonor(Unit *uVictim, uint32 groupsize, int32 honor, bool pvptoken)
{
    ...
    if (honor_f <= 0)
    {
        ...
        if (uVictim->GetTypeId() == TYPEID_PLAYER)
        {
            ...
            uint8 k_grey = Neo::XP::GetGrayLevel(k_level);
            ...
            honor_f = ceil(Neo::Honor::hk_honor_at_level_f(k_level) * (v_level - k_grey) / (k_level - k_grey));
...
{
    if (!m_timedquests.empty() && qInfo->HasFlag(QUEST_NEO_FLAGS_TIMED))
    {
        if (msg)
...
void Player::SendQuestReward(Quest const *pQuest, uint32 XP, Object * questGiver)
{
    ...
    data << 10 * Neo::Honor::hk_honor_at_level(getLevel(), pQuest->GetRewHonorableKills());
...
bool Player::_LoadFromDB(uint32 guid, SQLQueryHolder * holder, PreparedQueryResult & result)
{
    ...
    _LoadIntoDataField(result->GetCString(63), PLAYER_EXPLORED_ZONES_1, PLAYER_EXPLORED_ZONES_SIZE);
    _LoadIntoDataField(result->GetCString(66), PLAYER__FIELD_KNOWN_TITLES, KNOWN_TITLES_SIZE*2);
    ...
    SetByteValue(PLAYER_FIELD_BYTES, 2, result->GetUInt8(67));
    ...
    uint32 instanceId = result->GetUInt8(60);
...
#define RelocateToHomebind(){ mapId = m_homebindMapId; instanceId = 0; Relocate(m_homebindX, m_homebindY, m_homebindZ); }
    ...
    m_specsCount = result->GetUInt8(61);
    m_activeSpec = result->GetUInt8(62);
...
bool Player::Satisfy(AccessRequirement const* ar, uint32 target_map, bool report)
{
    if (!isGameMaster() && ar)
    {
        ...
        {
            GetSession()->SendAreaTriggerMessage("%s", GetSession()->GetNeoString(LANG_INSTANCE_CLOSED));
            return false;
        }
        ...
        if (LevelMin || LevelMax || missingItem || missingQuest || missingAchievement)
        {
            if (report)
            {
                if (missingQuest && !ar->questFailedText.empty())
                    ...
                    GetSession()->SendAreaTriggerMessage(GetSession()->GetNeoString(LANG_LEVEL_MINREQUIRED_AND_ITEM), LevelMin, sObjectMgr.GetItemPrototype(missingItem)->Name1);
                else if (LevelMin)
...
void Player::SaveToDB()
{
    ...
    ss << "REPLACE INTO characters (guid,account,name,race,class,gender,level,xp,money,playerBytes,playerBytes2,playerFlags,"
        ...
        "power4, power5, power6, power7, power8, power9, latency, speccount, activespec, exploredZones, equipmentCache, ammoId, knownTitles, actionBars) VALUES ("
        << GetGUIDLow() << ", "
...
void Player::HandleStealthedUnitsDetection()
{
    ...
    Neo::UnitListSearcher<Neo::AnyStealthedCheck > searcher(this, stealthedUnits, u_check);
...
void Player::UpdateObjectVisibility(bool forced)
{
    ...
    else
    {
        ...
        Neo::VisibleNotifier notifier(*this);
...
{
    uint8 v_level = pVictim->getLevel();
    uint8 k_grey  = Neo::XP::GetGrayLevel(getLevel());
...
bool Player::RewardPlayerAndGroupAtKill(Unit* pVictim)
{
    ...
    if (Group *pGroup = GetGroup())
    {
        ...
        if (member_with_max_level)
        {
            ...
            xp = (PvP || !not_gray_member_with_max_level || GetVehicle()) ? 0 : Neo::XP::Gain(not_gray_member_with_max_level, pVictim);
    ...
    {
        xp = (PvP || GetVehicle()) ? 0 : Neo::XP::Gain(this, pVictim);
