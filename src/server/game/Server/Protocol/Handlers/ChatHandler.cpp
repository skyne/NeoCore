...
void WorldSession::HandleMessagechatOpcode(WorldPacket & recv_data)
{
    ...
    else
    {
        ...
        if (!_player->CanSpeak())
        {
            ...
            SendNotification(GetNeoString(LANG_WAIT_BEFORE_SPEAKING),timeStr.c_str());
            return;
        }
    ...
    if (GetPlayer()->HasAura(1852) && type != CHAT_MSG_WHISPER)
    {
        ...
        SendNotification(GetNeoString(LANG_GM_SILENCE), GetPlayer()->GetName());
        return;
    }
    ...
    switch(type)
    {
        ...
        case CHAT_MSG_YELL:
        {
            ...
            {
                SendNotification(GetNeoString(LANG_SAY_REQ), sWorld.getIntConfig(CONFIG_CHAT_SAY_LEVEL_REQ));
                return;
            }
        ...
        case CHAT_MSG_WHISPER:
        {
            ...
            {
                SendNotification(GetNeoString(LANG_WHISPER_REQ), sWorld.getIntConfig(CONFIG_CHAT_WHISPER_LEVEL_REQ));
                return;
            }
        ...
        case CHAT_MSG_CHANNEL:
        {
            ...
            {
                SendNotification(GetNeoString(LANG_CHANNEL_REQ), sWorld.getIntConfig(CONFIG_CHAT_CHANNEL_LEVEL_REQ));
                return;
            }
        ...
        case CHAT_MSG_AFK:
        {
            ...
            if ((msg.empty() || !_player->isAFK()) && !_player->isInCombat())
            {
                ...
                {
                    if (msg.empty())
                        msg  = GetNeoString(LANG_PLAYER_AFK_DEFAULT);
                    _player->afkMsg = msg;
                }
        ...
        case CHAT_MSG_DND:
        {
            ...
            if (msg.empty() || !_player->isDND())
            {
                ...
                {
                    if (msg.empty())
                        msg  = GetNeoString(LANG_PLAYER_DND_DEFAULT);
                    _player->dndMsg = msg;
                }
...
void WorldSession::HandleTextEmoteOpcode(WorldPacket & recv_data)
{
    ...
    CellPair p = Neo::ComputeCellPair(GetPlayer()->GetPositionX(), GetPlayer()->GetPositionY());

    Cell cell(p);
    cell.data.Part.reserved = ALL_DISTRICT;
    cell.SetNoCreate();

    Neo::EmoteChatBuilder emote_builder(*GetPlayer(), text_emote, emoteNum, unit);
    Neo::LocalizedPacketDo<Neo::EmoteChatBuilder > emote_do(emote_builder);
    Neo::PlayerDistWorker<Neo::LocalizedPacketDo<Neo::EmoteChatBuilder > > emote_worker(GetPlayer(), sWorld.getFloatConfig(CONFIG_LISTEN_RANGE_TEXTEMOTE), emote_do);
    TypeContainerVisitor<Neo::PlayerDistWorker<Neo::LocalizedPacketDo<Neo::EmoteChatBuilder> >, WorldTypeMapContainer> message(emote_worker);
