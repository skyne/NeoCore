...
{
    char const* format = GetNeoString(string_id);
    if (format)
    {
        va_list ap;
        char szStr [1024];
...
const char * WorldSession::GetNeoString(int32 entry) const
{
    return sObjectMgr.GetNeoString(entry, GetSessionDbLocaleIndex());
}
...
void WorldSession::ReadAddonsInfo(WorldPacket &data)
{
    ...
    if (uncompress(const_cast<uint8*>(addonInfo.contents()), &uSize, const_cast<uint8*>(data.contents() + pos), data.size() - pos) == Z_OK)
    {
        ...
        for (uint32 i = 0; i < addonsCount; ++i)
        {
            ...
            sLog.outDebug("ADDON: Name: %s, Enabled: 0x%x, CRC: 0x%x, Unknown2: 0x%x\n", addonName.c_str(), enabled, crc, unk1);
