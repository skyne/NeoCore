...
{
    // NOT USED BY ANY SPELL OR USELESS OR IMPLEMENTED IN DIFFERENT WAY IN NEO
}
...
void Spell::EffectAddHonor(SpellEffIndex /*effIndex*/)
{
    ...
    if (damage <= 50)
    {
        uint32 honor_reward = Neo::Honor::hk_honor_at_level(unitTarget->getLevel(), damage);
...
void Spell::EffectSanctuary(SpellEffIndex /*effIndex*/)
{
    ...
    Neo::AnyUnfriendlyUnitInObjectRangeCheck u_check(unitTarget, unitTarget, m_caster->GetMap()->GetVisibilityDistance());
    Neo::UnitListSearcher<Neo::AnyUnfriendlyUnitInObjectRangeCheck> searcher(unitTarget, targets, u_check);
