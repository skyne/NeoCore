...
WorldObject* Spell::FindCorpseUsing()
{
    ...
    CellPair p(Neo::ComputeCellPair(m_caster->GetPositionX(), m_caster->GetPositionY()));
    Cell cell(p);
    ...
    Neo::WorldObjectSearcher<T> searcher(m_caster, result, u_check);

    TypeContainerVisitor<Neo::WorldObjectSearcher<T>, GridTypeMapContainer > grid_searcher(searcher);
    cell.Visit(p, grid_searcher, *m_caster->GetMap(), *m_caster, max_range);

    if (!result)
    {
        TypeContainerVisitor<Neo::WorldObjectSearcher<T>, WorldTypeMapContainer > world_searcher(searcher);
                    ...
                    switch(m_spellInfo->Id)
                    {
                        ...
                        case 54044:                         // Carrion Feeder
                        {
                            ...
                            if (m_spellInfo->Id == 20577)
                                result = FindCorpseUsing<Neo::CannibalizeObjectCheck>();
                            else
                                result = FindCorpseUsing<Neo::CarrionFeederObjectCheck>();
...
void Spell::SearchChainTarget(std::list<Unit*> &TagUnitMap, float max_range, uint32 num, SpellTargets TargetType)
{
    ...
    while (num)
    {
        ...
        {
            tempUnitMap.sort(Neo::ObjectDistanceOrderPred(cur));
            next = tempUnitMap.begin();
...
void Spell::SearchAreaTarget(std::list<Unit*> &TagUnitMap, float radius, SpellNotifyPushType type, SpellTargets TargetType, uint32 entry)
{
    ...
    Neo::SpellNotifierCreatureAndPlayer notifier(m_caster, TagUnitMap, radius, type, TargetType, pos, entry, requireDeadTarget);
...
void Spell::SearchGOAreaTarget(std::list<GameObject*> &TagGOMap, float radius, SpellNotifyPushType type, SpellTargets TargetType, uint32 entry)
{
    ...
    Neo::GameObjectInRangeCheck check(pos->m_positionX, pos->m_positionY, pos->m_positionZ, radius + 50, entry);
    Neo::GameObjectListSearcher<Neo::GameObjectInRangeCheck> searcher(m_caster, TagGOMap, check);
...
WorldObject* Spell::SearchNearbyTarget(float range, SpellTargets TargetType)
{
    switch(TargetType)
    {
        ...
        {
            Unit *target = NULL;
            Neo::AnyUnfriendlyUnitInObjectRangeCheck u_check(m_caster, m_caster, range);
            Neo::UnitLastSearcher<Neo::AnyUnfriendlyUnitInObjectRangeCheck> searcher(m_caster, target, u_check);
        ...
        {
            Unit *target = NULL;
            Neo::AnyFriendlyUnitInObjectRangeCheck u_check(m_caster, m_caster, range);
            Neo::UnitLastSearcher<Neo::AnyFriendlyUnitInObjectRangeCheck> searcher(m_caster, target, u_check);
            m_caster->VisitNearbyObject(range, searcher);
            return target;
        }
    }
    ...
    else if (pushType)
    {
            ...
            case SPELL_TARGETS_ENTRY:
            {
                ...
                else
                {
                    ...
                    switch (m_spellInfo->Id)
                    {
                        ...
                        {
                            if (WorldObject* result = FindCorpseUsing<Neo::RaiseDeadObjectCheck> ())
                            {
                                switch(result->GetTypeId())
                                {
                                    case TYPEID_UNIT:
                        ...
                        case 51328:
                                    ...
                                    && m_targets.getUnitTarget()->GetDisplayId() == m_targets.getUnitTarget()->GetNativeDisplayId()))))
                            {
                                ...
                                WorldObject* result = FindCorpseUsing <Neo::ExplodeCorpseObjectCheck> ();
        ...
        if (!unitList.empty())
        {
            ...
            if (maxSize && power != -1)
            {
                if (Powers(power) == POWER_HEALTH)
                {
                    ...
                    {
                        unitList.sort(Neo::HealthPctOrderPred());
                ...
                else
                {
                    ...
                    if (unitList.size() > maxSize)
                    {
                        unitList.sort(Neo::PowerPctOrderPred((Powers)power));
                        unitList.resize(maxSize);
                    }
                }
            }
            ...
            if (uint32 maxTargets = m_spellValue->MaxAffectedTargets)
            {
                ...
                Neo::RandomResizeList(unitList, maxTargets);
            }
            else
            {
                switch (m_spellInfo->Id)
        ...
        if (!gobjectList.empty())
        {
            if (uint32 maxTargets = m_spellValue->MaxAffectedTargets)
            {
                ...
                Neo::RandomResizeList(gobjectList, maxTargets);
            }
                    ...
                    switch(i_spellST->second.type)
                    {
                        case SPELL_TARGET_TYPE_GAMEOBJECT:
                        {
                            ...
                            if (i_spellST->second.targetEntry)
                            {
                                ...
                                Neo::NearestGameObjectEntryInObjectRangeCheck go_check(*m_caster,i_spellST->second.targetEntry,range);
                                Neo::GameObjectLastSearcher<Neo::NearestGameObjectEntryInObjectRangeCheck> checker(m_caster, p_GameObject,go_check);

                                TypeContainerVisitor<Neo::GameObjectLastSearcher<Neo::NearestGameObjectEntryInObjectRangeCheck>, GridTypeMapContainer > object_checker(checker);
                        ...
                        default:
                        {
                            ...
                            Neo::NearestCreatureEntryWithLiveStateInObjectRangeCheck u_check(*m_caster,i_spellST->second.targetEntry,i_spellST->second.type != SPELL_TARGET_TYPE_DEAD,range);
                            Neo::CreatureLastSearcher<Neo::NearestCreatureEntryWithLiveStateInObjectRangeCheck> searcher(m_caster, p_Creature, u_check);

                            TypeContainerVisitor<Neo::CreatureLastSearcher<Neo::NearestCreatureEntryWithLiveStateInObjectRangeCheck>, GridTypeMapContainer >  grid_creature_searcher(searcher);
...
SpellCastResult Spell::CheckItems()
{
    ...
    if (m_spellInfo->RequiresSpellFocus)
    {
        ...
        GameObject* ok = NULL;
        Neo::GameObjectFocusCheck go_check(m_caster,m_spellInfo->RequiresSpellFocus);
        Neo::GameObjectSearcher<Neo::GameObjectFocusCheck> checker(m_caster, ok, go_check);

        TypeContainerVisitor<Neo::GameObjectSearcher<Neo::GameObjectFocusCheck>, GridTypeMapContainer > object_checker(checker);
...
void Spell::SelectTrajTargets()
{
    ...
    unitList.sort(Neo::ObjectDistanceOrderPred(m_caster));
