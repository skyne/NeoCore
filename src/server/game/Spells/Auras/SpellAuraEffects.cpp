...
// such checks will speedup neo change amount/send for client operations
            ...
            switch (GetSpellProto()->Id)
            {
                ...
                case 51690:
                {
                    ...
                    UnitList targets;
                    {
                        ...
                        Neo::AnyUnfriendlyVisibleUnitInObjectRangeCheck u_check(caster, caster, radius);
                        Neo::UnitListSearcher<Neo::AnyUnfriendlyVisibleUnitInObjectRangeCheck> checker(caster,targets, u_check);

                        TypeContainerVisitor<Neo::UnitListSearcher<Neo::AnyUnfriendlyVisibleUnitInObjectRangeCheck>, GridTypeMapContainer > grid_object_checker(checker);
                        TypeContainerVisitor<Neo::UnitListSearcher<Neo::AnyUnfriendlyVisibleUnitInObjectRangeCheck>, WorldTypeMapContainer > world_object_checker(checker);
...
void AuraEffect::HandleFeignDeath(AuraApplication const * aurApp, uint8 mode, bool apply) const
{
    ...
    if (apply)
    {
        ...
        */
        ...
        UnitList targets;
        Neo::AnyUnfriendlyUnitInObjectRangeCheck u_check(target, target, target->GetMap()->GetVisibilityDistance());
        Neo::UnitListSearcher<Neo::AnyUnfriendlyUnitInObjectRangeCheck> searcher(target, targets, u_check);
