...
#ifndef NEOCORE_QUEST_H
#define NEOCORE_QUEST_H
...
enum __QuestFlags
{
    ...
    QUEST_NEO_FLAGS_REPEATABLE           = 0x00100000,   // Set by 1 in SpecialFlags from DB
    QUEST_NEO_FLAGS_EXPLORATION_OR_EVENT = 0x00200000,   // Set by 2 in SpecialFlags from DB (if reequired area explore, spell SPELL_EFFECT_QUEST_COMPLETE casting, table `*_script` command SCRIPT_COMMAND_QUEST_EXPLORED use, set from script)
    QUEST_NEO_FLAGS_AUTO_ACCEPT          = 0x00400000,  // Set by 4 in SpecialFlags in DB if the quest is to be auto-accepted.

    QUEST_NEO_FLAGS_DB_ALLOWED = 0xFFFFF | QUEST_NEO_FLAGS_REPEATABLE | QUEST_NEO_FLAGS_EXPLORATION_OR_EVENT | QUEST_NEO_FLAGS_AUTO_ACCEPT,

    // Neo flags for internal use only
    QUEST_NEO_FLAGS_DELIVER              = 0x04000000,   // Internal flag computed only
    QUEST_NEO_FLAGS_SPEAKTO              = 0x08000000,   // Internal flag computed only
    QUEST_NEO_FLAGS_KILL_OR_CAST         = 0x10000000,   // Internal flag computed only
    QUEST_NEO_FLAGS_TIMED                = 0x20000000,   // Internal flag computed only
};
