    ...
    if (IsBeingEscorted && !Defend)
        ...
        if (!InCombat)
        {
            ...
            m_creature->GetPosition(LastPos.x, LastPos.y, LastPos.z);
             ...
             debug_log("NeoScript: EscortAI has entered combat and stored last location.");
            }
...
void npc_escortAI::EnterEvadeMode()
{
    ...
    if (IsBeingEscorted)
    {
        debug_log("NeoScript: EscortAI has left combat and is now returning to last point.");
        Returning = true;
        m_creature->GetMotionMaster()->MovementExpired();
...
void npc_escortAI::UpdateAI(const uint32 diff)
{
        ...
        if (WaitTimer <= diff)
    {
        if (ReconnectWP)
        {
            ...
            if( !IsOnHold )
            {
                ...
                debug_log("NeoScript: EscortAI Reconnect WP is: %u, %f, %f, %f", CurrentWP->id, CurrentWP->x, CurrentWP->y, CurrentWP->z);
                WaitTimer = 0;
                ReconnectWP = false;
                return;
            }
        ...
        if (CurrentWP == WaypointList.end())
        {
            if(DespawnAtEnd)
            {
                debug_log("NeoScript: EscortAI reached end of waypoints");
                ...
                debug_log("NeoScript: EscortAI reached end of waypoints with Despawn off");
                IsBeingEscorted = false;
                WaitTimer = 0;
                return;
            }
        ...
        {
            m_creature->GetMotionMaster()->MovePoint(CurrentWP->id, CurrentWP->x, CurrentWP->y, CurrentWP->z );
            debug_log("NeoScript: EscortAI Next WP is: %u, %f, %f, %f", CurrentWP->id, CurrentWP->x, CurrentWP->y, CurrentWP->z);
            WaitTimer = 0;
        }
    }else WaitTimer -= diff;
    }
        ...
        if (PlayerTimer < diff)
    {
        ...
        if (DespawnAtFar && (!p || m_creature->GetDistance(p) > GetMaxPlayerDistance()))
        {
            ...
            debug_log("NeoScript: EscortAI Evaded back to spawn point because player was to far away or not found");
...
void npc_escortAI::MovementInform(uint32 type, uint32 id)
{
    ...
    if (id == WP_LAST_POINT)
    {
        debug_log("NeoScript: EscortAI has returned to original position before combat");
        ReconnectWP = true;
        Returning = false;
        WaitTimer = 1;
    ...
    }else
    {
        ...
        debug_log("NeoScript: EscortAI Waypoint %d reached", CurrentWP->id);
...
void npc_escortAI::Start(bool bAttack, bool bDefend, bool bRun, uint64 pGUID)
{
    ...
    debug_log("NeoScript: EscortAI started with %d waypoints. Attack = %d, Defend = %d, Run = %d, PlayerGUID = %d", WaypointList.size(), Attack, Defend, Run, PlayerGUID);
    ...
    //Start WP
    m_creature->GetMotionMaster()->MovePoint(CurrentWP->id, CurrentWP->x, CurrentWP->y, CurrentWP->z );
    debug_log("NeoScript: EscortAI Next WP is: %d, %f, %f, %f", CurrentWP->id, CurrentWP->x, CurrentWP->y, CurrentWP->z);
