...
struct NEO_DLL_DECL instance_stratholme : public ScriptedInstance
{
    ...
    Player* GetPlayerInMap()
    {
        ...
        debug_log("NeoScript: Instance Stratholme: GetPlayerInMap, but PlayerList is empty!");
        return NULL;
    }
    ...
    bool StartSlaugtherSquare()
    {
        ...
        debug_log("NeoScript: Instance Stratholme: Cannot open slaugther square yet.");
        return false;
    }
    ...
    void SetData(uint32 type, uint32 data)
    {
        ...
        if (!player)
        {
            debug_log("NeoScript: Instance Stratholme: SetData (Type: %u Data %u) cannot find any player.", type, data);
            return;
        }
        ...
        case TYPE_BARON_RUN:
            ...
            case IN_PROGRESS:
                ...
                debug_log("NeoScript: Instance Stratholme: Baron run in progress.");
                break;
            case FAIL:
        ...
        case TYPE_RAMSTEIN:
            if (data == IN_PROGRESS)
            {
                ...
                if (!count)
                {
                    ...
                    debug_log("NeoScript: Instance Stratholme: Ramstein spawned.");
                } else debug_log("NeoScript: Instance Stratholme: %u Abomnation left to kill.",count);
            }
            if (data == DONE)
            {
                SlaugtherSquare_Timer = 300000;
                debug_log("NeoScript: Instance Stratholme: Slaugther event will continue in 5 minutes.");
            }
            Encounter[4] = data;
            break;
        case TYPE_BARON:
    ...
    void Update(uint32 diff)
    {
        if (BaronRun_Timer)
        {
            if (BaronRun_Timer <= diff)
            {
                ...
                BaronRun_Timer = 0;
                debug_log("NeoScript: Instance Stratholme: Baron run event reached end. Event has state %u.",GetData(TYPE_BARON_RUN));
        ...
        if (SlaugtherSquare_Timer)
        {
            if (SlaugtherSquare_Timer <= diff)
            {
                if (Player *p = GetPlayerInMap())
                {
                    ...
                    debug_log("NeoScript: Instance Stratholme: Black guard sentries spawned. Opening gates to baron.");
                }
                SlaugtherSquare_Timer = 0;
