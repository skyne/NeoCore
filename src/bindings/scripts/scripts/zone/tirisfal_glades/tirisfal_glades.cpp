...
struct NEO_DLL_DECL npc_calvin_montagueAI : public ScriptedAI
{
...
GameObject* SearchMausoleumGo(Unit *source, uint32 entry, float range)
{
    ...
    Neo::NearestGameObjectEntryInObjectRangeCheck go_check(*source, entry, range);
    Neo::GameObjectLastSearcher<Neo::NearestGameObjectEntryInObjectRangeCheck> searcher(pGo, go_check);

    TypeContainerVisitor<Neo::GameObjectLastSearcher<Neo::NearestGameObjectEntryInObjectRangeCheck>, GridTypeMapContainer> go_searcher(searcher);
