...
struct NEO_DLL_DECL boss_nalorakkAI : public ScriptedAI
{
    ...
    void SendAttacker(Unit* target)
    {
...

        {
            ...
            Neo::AllFriendlyCreaturesInGrid check(m_creature);
            Neo::CreatureListSearcher<Neo::AllFriendlyCreaturesInGrid> searcher(templist, check);

            TypeContainerVisitor<Neo::CreatureListSearcher<Neo::AllFriendlyCreaturesInGrid>, GridTypeMapContainer> cSearcher(searcher);
    ...
    void UpdateAI(const uint32 diff)
    {
        ...
        if(!inBearForm)
        {
            ...
            if(Surge_Timer < diff)
            {
                ...
                Unit *target = SelectUnit(SELECT_TARGET_RANDOM, 1, 45, true);
                if(target)
