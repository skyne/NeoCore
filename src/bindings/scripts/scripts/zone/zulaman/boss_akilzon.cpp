...
struct NEO_DLL_DECL boss_akilzonAI : public ScriptedAI
{
    ...
    void HandleStormSequence(Unit *Cloud) // 1: begin, 2-9: tick, 10: end
    {
        if(StormCount < 10 && StormCount > 1)
        {
            ...
            {
                Neo::AnyAoETargetUnitInObjectRangeCheck u_check(m_creature, m_creature, 999);
                Neo::UnitListSearcher<Neo::AnyAoETargetUnitInObjectRangeCheck> searcher(tempUnitMap, u_check);

                TypeContainerVisitor<Neo::UnitListSearcher<Neo::AnyAoETargetUnitInObjectRangeCheck>, WorldTypeMapContainer > world_unit_searcher(searcher);
                TypeContainerVisitor<Neo::UnitListSearcher<Neo::AnyAoETargetUnitInObjectRangeCheck>, GridTypeMapContainer >  grid_unit_searcher(searcher);
...
struct NEO_DLL_DECL mob_soaring_eagleAI : public ScriptedAI
{
