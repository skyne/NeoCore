...
struct NEO_DLL_DECL aqsentinelAI;
class NEO_DLL_DECL SentinelAbilityAura : public Aura
{
    public:
        ~SentinelAbilityAura();
...
struct NEO_DLL_DECL aqsentinelAI : public ScriptedAI
{
    uint32 ability;
    int abselected;
    ...
    {
        CellPair p(Neo::ComputeCellPair(nears->GetPositionX(), nears->GetPositionY()));
        Cell cell(p);
        ...
        Neo::CreatureListSearcher<NearbyAQSentinel> searcher(assistList, u_check);
        TypeContainerVisitor<Neo::CreatureListSearcher<NearbyAQSentinel>, GridTypeMapContainer >  grid_creature_searcher(searcher);
