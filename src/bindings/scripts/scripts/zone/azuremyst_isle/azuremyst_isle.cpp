...
struct NEO_DLL_DECL npc_draenei_survivorAI : public ScriptedAI
{
...
struct NEO_DLL_DECL npc_engineer_spark_overgrindAI : public ScriptedAI
{
...
struct NEO_DLL_DECL npc_injured_draeneiAI : public ScriptedAI
{
...
struct NEO_DLL_DECL npc_magwinAI : public npc_escortAI
{
...
bool GossipSelect_npc_susurrus(Player *player, Creature *_Creature, uint32 sender, uint32 action )
{
    if (action == GOSSIP_ACTION_INFO_DEF)
    {
        ...
        player->ActivateTaxiPathTo(nodes,11686);            //TaxiPath 506. Using invisible model, possible NEO must allow 0(from dbc) for cases like this.
    }
    return true;
}
...
struct NEO_DLL_DECL npc_geezleAI : public ScriptedAI
{
    ...
    void DespawnNagaFlag(bool despawn)
    {
        ...
        Neo::AllGameObjectsWithEntryInGrid go_check(GO_NAGA_FLAG);
        Neo::GameObjectListSearcher<Neo::AllGameObjectsWithEntryInGrid> go_search(FlagList, go_check);
        TypeContainerVisitor
            <Neo::GameObjectListSearcher<Neo::AllGameObjectsWithEntryInGrid>, GridTypeMapContainer> go_visit(go_search);
...
struct NEO_DLL_DECL mob_nestlewood_owlkinAI : public ScriptedAI
{
