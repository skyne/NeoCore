...
struct NEO_DLL_DECL npc_manaforge_control_consoleAI : public ScriptedAI
{
...
//TODO: clean up this workaround when Neo adds support to do it properly (with gossip selections instead of instant summon)
...
struct NEO_DLL_DECL npc_commander_dawnforgeAI : public ScriptedAI
{
    ...
    Creature* SelectCreatureInGrid(uint32 entry, float range)
    {
        ...
        Neo::NearestCreatureEntryWithLiveStateInObjectRangeCheck creature_check(*m_creature, entry, true, range);
...
Creature* SearchDawnforge(Player *source, uint32 entry, float range)
{
    ...
    CellPair pair(Neo::ComputeCellPair(source->GetPositionX(), source->GetPositionY()));
    Cell cell(pair);
    cell.data.Part.reserved = ALL_DISTRICT;
    cell.SetNoCreate();

    Neo::NearestCreatureEntryWithLiveStateInObjectRangeCheck creature_check(*source, entry, true, range);
    Neo::CreatureLastSearcher<Neo::NearestCreatureEntryWithLiveStateInObjectRangeCheck> searcher(pCreature, creature_check);
...
struct NEO_DLL_DECL mob_phase_hunterAI : public ScriptedAI
{
...
struct NEO_DLL_DECL npc_bessyAI : public npc_escortAI
{
