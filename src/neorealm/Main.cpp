...
# define _REALMDCONFVERSION 20100920
#endif

#ifndef _NEO_REALM_CONFIG
# define _NEO_REALM_CONFIG  "neorealm.conf"
#endif //_NEO_REALM_CONFIG

#ifdef WIN32
#include "ServiceWin32.h"
char serviceName[] = "realmd";
char serviceLongName[] = "Neo realm service";
...
extern int main(int argc, char **argv)
{
    ...
    char const* cfg_file = _NEO_REALM_CONFIG;
    int c=1;
    while( c < argc )
    {
    ...
    {
        sLog.outError("*****************************************************************************");
        sLog.outError(" WARNING: Your neorealm.conf version indicates your conf file is out of date!");
	...
	sLog.outString( "_______________________________________________________");
	sLog.outString( "    _     _               ____                         ");
	sLog.outString( "    /|   /                /    )               /       ");
	sLog.outString( "---/-| -/-----__----__---/___ /----__----__---/---_--_-");
  	sLog.outString( "  /  | /    /___) /   ) /    |   /___) /   ) /   / /  ) ");
	sLog.outString( "_/___|/____(___ _(___/_/_____|__(___ _(___(_/___/_/__/_");
    ...
    {
        sLog.outError( "Neo realm can not bind to %s:%d",bind_ip.c_str(), rmport );
        return 1;
    }
    ...
    #ifdef WIN32
    {
        ...
        if(Prio)
        {
            if(SetPriorityClass(hProcess,HIGH_PRIORITY_CLASS))
                sLog.outString("NeoRealm process priority class set to HIGH");
            else
