...
typedef struct AUTH_LOGON_PROOF_S2    //5875 && 6005
{
    uint8   cmd;
    uint8   error;
    uint8   M2[20];
...
bool AuthSocket::_HandleLogonChallenge()
{
	...
	LoginDatabase.escape_string(host);
	...
	if(host_query != " " || host_query != "")
		result2 = LoginDatabase.PQuery(  "SELECT * FROM host_banned WHERE host REGEXP '%s'",host_query.c_str());

	QueryResult_AutoPtr result3 = LoginDatabase.PQuery("SELECT 1 FROM account WHERE last_ip = '%s' AND active_realm_id > 0",address.c_str());
	QueryResult_AutoPtr countresult = LoginDatabase.PQuery("SELECT count FROM allowed_ip_count WHERE ip = '%s'",address.c_str());
	bool TooManyIP = false;
	int count = sConfig.GetIntDefault("Max.Allowed.Accounts.In.IP",50);

	if(countresult)
	{
		Field *fields = countresult->Fetch();
		count = fields[0].GetInt16();
	}
	...
	{
		pkt << (uint8)REALM_AUTH_ACCOUNT_BANNED;
		sLog.outBasic("[AuthChallenge] Banned host %s tries to login!",GetRemoteHostname().c_str ());
		SendBuf((char const*)pkt.contents(), pkt.size());
		return false;
	}
	else if(result3)
	{	
		uint64 onlineacc = result3->GetRowCount();		

		if(onlineacc >= count)
		{
			pkt << (uint8)REALM_AUTH_SERVER_FULL;
			sLog.outBasic("[AuthChallenge] Too many account tries to login from IP: %s, now online:%i maximum: %i!",GetRemoteAddress().c_str(),onlineacc,count);
			SendBuf((char const*)pkt.contents(), pkt.size());
			return false;
		}
	...
	result = LoginDatabase.PQuery("SELECT sha_pass_hash,id,locked,last_ip,host FROM account WHERE username = '%s'",_safelogin.c_str ());
	if( result )
	{
...
bool AuthSocket::_HandleLogonProof()
{
    ...
    if(!valid_version)
    {
		...
		/*std::string patches = sConfig.GetStringDefault("Patches.Folder","patch");
		
        sprintf(tmp,"%s/%d%s.mpq",patches.c_str(),_build, _localizationName.c_str());*/
    ...
    if (!memcmp(M.AsByteArray(), lp.M1, 20))
    {
		...
		LoginDatabase.PExecute("UPDATE account SET sessionkey = '%s', last_ip = '%s',host= '%s', last_login = NOW(), locale = '%u', failed_logins = 0 WHERE username = '%s'", K_hex, GetRemoteAddress().c_str(),GetRemoteHostname().c_str(), GetLocaleByName(_localizationName), _safelogin.c_str() );
            ...
            if(QueryResult_AutoPtr loginfail = LoginDatabase.PQuery("SELECT id, failed_logins FROM account WHERE username = '%s'", _safelogin.c_str()))
            {
                ...
                if( failed_logins >= MaxWrongPassCount )
                {
                    ...
                    if(WrongPassBanType)
                    {
                        ...
                        LoginDatabase.PExecute("INSERT INTO account_banned VALUES ('%u',UNIX_TIMESTAMP(),UNIX_TIMESTAMP()+'%u','Neo realmd','Failed login autoban',1)",
                    ...
                    else
                    {
                        ...
                        LoginDatabase.PExecute("INSERT INTO ip_banned VALUES ('%s',UNIX_TIMESTAMP(),UNIX_TIMESTAMP()+'%u','Neo realmd','Failed login autoban')",
