...
void BattleGroundMgr::Update(time_t diff)
{
    ...
    if (m_AutoDistributePoints && (!(sWorld.getConfig(CONFIG_SERVER_LOCKDOWN))
		// lockdown is off
		|| (sWorld.getConfig(CONFIG_SERVER_MAINTENANCE_TASKS) & MAINTENANCE_TASK_DISTRIBUTE_ARENA_POINTS) == 0))
		// or on but not affecting arena point distribution
	{
        if (m_AutoDistributionTimeChecker < diff)
        {
            if (time(NULL) > m_NextAutoDistributionTime)
            {
                ...
                CharacterDatabase.PExecute("UPDATE `saved_variables` SET `value` = '"UI64FMTD"' WHERE `name` = 'NextArenaPointDistributionTime'",(uint64)m_NextAutoDistributionTime);
            }
...
void BattleGroundMgr::InitAutomaticArenaPointDistribution()
{
    if (sWorld.getConfig(CONFIG_ARENA_AUTO_DISTRIBUTE_POINTS)
		&& (!(sWorld.getConfig(CONFIG_SERVER_LOCKDOWN))
		// lockdown is off
		|| (sWorld.getConfig(CONFIG_SERVER_MAINTENANCE_TASKS) & MAINTENANCE_TASK_DISTRIBUTE_ARENA_POINTS) == 0))
		// or on but not affecting arena point distribution
	{
        sLog.outDebug("Initializing Automatic Arena Point Distribution");
        QueryResult_AutoPtr result = CharacterDatabase.Query("SELECT `value` FROM `saved_variables` WHERE `name` = 'NextArenaPointDistributionTime'");
        if (!result)
        {
            sLog.outDebug("Battleground: Next arena point distribution time not found in SavedVariables, reseting it now.");
            m_NextAutoDistributionTime = time(NULL) + BATTLEGROUND_ARENA_POINT_DISTRIBUTION_DAY * sWorld.getConfig(CONFIG_ARENA_AUTO_DISTRIBUTE_INTERVAL_DAYS);
            CharacterDatabase.PExecute("REPLACE INTO `saved_variables`(`name`,`value`) VALUES ('NextArenaPointDistributionTime', '"UI64FMTD"' )",(uint64)m_NextAutoDistributionTime);
        }
        else
