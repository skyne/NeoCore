...
void CreatureEventAI::ProcessAction(CreatureEventAI_Action const& action, uint32 rnd, uint32 EventId, Unit* pActionInvoker)
{
    switch (action.type)
    {
        ...
        case ACTION_T_SUMMON_GO:
        {
            ...
            if (!pObject)
            {
                sLog.outErrorDb("NeoScript: EventAI failed to spawn object %u. Spawn event %d is on creature %d", action.raw.param1, EventId, m_creature->GetEntry());
            }
            break;
        }
...
Unit* CreatureEventAI::DoSelectLowestHpFriendly(float range, uint32 MinHPDiff)
{
    ...
    */
    TypeContainerVisitor<Neo::UnitLastSearcher<Neo::MostHPMissingInRange>, GridTypeMapContainer >  grid_unit_searcher(searcher);
...
void CreatureEventAI::DoFindFriendlyCC(std::list<Creature*>& _list, float range)
{
    ...
    Neo::CreatureListSearcher<Neo::FriendlyCCedInRange> searcher(_list, u_check);

    TypeContainerVisitor<Neo::CreatureListSearcher<Neo::FriendlyCCedInRange>, GridTypeMapContainer >  grid_creature_searcher(searcher);
...
void CreatureEventAI::DoFindFriendlyMissingBuff(std::list<Creature*>& _list, float range, uint32 spellid)
{
    ...
    Neo::CreatureListSearcher<Neo::FriendlyMissingBuffInRange> searcher(_list, u_check);

    TypeContainerVisitor<Neo::CreatureListSearcher<Neo::FriendlyMissingBuffInRange>, GridTypeMapContainer >  grid_creature_searcher(searcher);
