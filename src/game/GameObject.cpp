    ...
    switch (m_lootState)
    {
        ...
        case GO_READY:
        {
            ...
            if (goInfo->type == GAMEOBJECT_TYPE_TRAP)
            {
                ...
                {
                    Neo::AnyUnfriendlyNoTotemUnitInObjectRangeCheck u_check(this, owner, radius);
                    Neo::UnitSearcher<Neo::AnyUnfriendlyNoTotemUnitInObjectRangeCheck> checker(ok, u_check);

                    CellLock<GridReadGuard> cell_lock(cell, p);

                    TypeContainerVisitor<Neo::UnitSearcher<Neo::AnyUnfriendlyNoTotemUnitInObjectRangeCheck>, GridTypeMapContainer > grid_object_checker(checker);
                    cell_lock->Visit(cell_lock, grid_object_checker, *MapManager::Instance().GetMap(GetMapId(), this));

                    // or unfriendly player/pet
                    if (!ok)
                    {
                        TypeContainerVisitor<Neo::UnitSearcher<Neo::AnyUnfriendlyNoTotemUnitInObjectRangeCheck>, WorldTypeMapContainer > world_object_checker(checker);
                ...
                else                                        // environmental trap
                {
                    ...
                    // affect only players
                    Player* p_ok = NULL;
                    Neo::AnyPlayerInObjectRangeCheck p_check(this, radius);
                    Neo::PlayerSearcher<Neo::AnyPlayerInObjectRangeCheck>  checker(p_ok, p_check);

                    CellLock<GridReadGuard> cell_lock(cell, p);

                    TypeContainerVisitor<Neo::PlayerSearcher<Neo::AnyPlayerInObjectRangeCheck>, WorldTypeMapContainer > world_object_checker(checker);
                    cell_lock->Visit(cell_lock, world_object_checker, *MapManager::Instance().GetMap(GetMapId(), this));
                    ok = p_ok;
                }
...
void GameObject::TriggeringLinkedGameObject(uint32 trapEntry, Unit* target)
{
    ...
    GameObject* trapGO = NULL;
    {
        ...
        Neo::NearestGameObjectEntryInObjectRangeCheck go_check(*target,trapEntry,range);
        Neo::GameObjectLastSearcher<Neo::NearestGameObjectEntryInObjectRangeCheck> checker(trapGO,go_check);
...
GameObject* GameObject::LookupFishingHoleAround(float range)
{
    ...
    Cell cell(p);
    cell.data.Part.reserved = ALL_DISTRICT;
    Neo::NearestGameObjectFishingHole u_check(*this, range);
    Neo::GameObjectSearcher<Neo::NearestGameObjectFishingHole> checker(ok, u_check);

    CellLock<GridReadGuard> cell_lock(cell, p);

    TypeContainerVisitor<Neo::GameObjectSearcher<Neo::NearestGameObjectFishingHole>, GridTypeMapContainer > grid_object_checker(checker);
    cell_lock->Visit(cell_lock, grid_object_checker, *MapManager::Instance().GetMap(GetMapId(), this));
