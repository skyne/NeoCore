    ...
    if (m_caster_guid == m_target->GetGUID())
    {
        ...
        if (!caster->hasUnitState(UNIT_STAT_ISOLATED) )
        {
            ...
            switch(m_areaAuraType)
            {
                ...
                {
                    Neo::AnyFriendlyUnitInObjectRangeCheck u_check(caster, caster, m_radius);
                    Neo::UnitListSearcher<Neo::AnyFriendlyUnitInObjectRangeCheck> searcher(targets, u_check);
                    caster->VisitNearbyObject(m_radius, searcher);
                    break;
                }
                case AREA_AURA_ENEMY:
                {
                    Neo::AnyAoETargetUnitInObjectRangeCheck u_check(caster, caster, m_radius); // No GetCharmer in searcher
                    Neo::UnitListSearcher<Neo::AnyAoETargetUnitInObjectRangeCheck> searcher(targets, u_check);
                    caster->VisitNearbyObject(m_radius, searcher);
                    break;
                }
...
void Aura::HandleChannelDeathItem(bool apply, bool Real)
{
    if (Real && !apply)
    {
        ...
        if (spellInfo->EffectItemType[m_effIndex] == 6265 &&
            (victim->getLevel() <= Neo::XP::GetGrayLevel(caster->getLevel()) ||
...
void Aura::HandleFeignDeath(bool apply, bool Real)
{
    ...
    if (apply )
    {
        ...
        */
        ...
        std::list<Unit*> targets;
        Neo::AnyUnfriendlyUnitInObjectRangeCheck u_check(m_target, m_target, World::GetMaxVisibleDistance());
...
void Aura::HandleAuraModIncreaseFlightSpeed(bool apply, bool Real)
{
    ...
    if (m_modifier.m_auraname == SPELL_AURA_MOD_INCREASE_FLIGHT_SPEED)
    {
        ...
        if (apply)
        {
            m_target->ToPlayer()->SetCanFly(true);
            data.Initialize(SMSG_MOVE_SET_CAN_FLY, 12);
        }
        else
        {
            data.Initialize(SMSG_MOVE_UNSET_CAN_FLY, 12);
            m_target->ToPlayer()->SetCanFly(false);
        }
        data.append(m_target->GetPackGUID());
