...
//                SpawnBGCreature(i, RESPAWN_IMMEDIATELY);
        ...
        {
            m_Events |= 0x04;
            SendMessageToAll(GetNeoString(LANG_BG_WS_ONE_MINUTE));
        }
        // After 1,5 minute, warning is signalled
        else if (GetStartDelayTime() <= START_DELAY2 && !(m_Events & 0x08))
        {
            m_Events |= 0x08;
            SendMessageToAll(GetNeoString(LANG_BG_WS_HALF_MINUTE));
        }
        ...
        else if (GetStartDelayTime() < 0 && !(m_Events & 0x10))
        {
            ...
            SendMessageToAll(GetNeoString(LANG_BG_WS_BEGIN));
...
void BattleGroundWS::RespawnFlag(uint32 Team, bool captured)
{
    ...
    if (captured)
    {
        ...
        SendMessageToAll(GetNeoString(LANG_BG_WS_F_PLACED));
...
void BattleGroundWS::RespawnFlagAfterDrop(uint32 team)
{
    ...
    {
        SpawnBGObject(BG_WS_OBJECT_A_FLAG, RESPAWN_IMMEDIATELY);
        SendMessageToAll(GetNeoString(LANG_BG_WS_ALLIANCE_FLAG_RESPAWNED));
    }
    else
    {
        SpawnBGObject(BG_WS_OBJECT_H_FLAG, RESPAWN_IMMEDIATELY);
        SendMessageToAll(GetNeoString(LANG_BG_WS_HORDE_FLAG_RESPAWNED));
    }
...
void BattleGroundWS::EventPlayerCapturedFlag(Player *Source)
{
    ...
    if (Source->GetTeam() == ALLIANCE)
    {
        ...
        message = GetNeoString(LANG_BG_WS_CAPTURED_HF);
    ...
    else
    {
        ...
        if (m_FlagDebuffState == 2)
          Source->RemoveAurasDueToSpell(WS_SPELL_BRUTAL_ASSAULT);
        message = GetNeoString(LANG_BG_WS_CAPTURED_AF);
...
void BattleGroundWS::EventPlayerDroppedFlag(Player *Source)
{
    ...
    if (Source->GetTeam() == ALLIANCE)
    {
        ...
        if (GetHordeFlagPickerGUID() == Source->GetGUID())
        {
            ...
            message = GetNeoString(LANG_BG_WS_DROPPED_HF);
            type = CHAT_MSG_BG_SYSTEM_HORDE;
    ...
    else
    {
        ...
        if (GetAllianceFlagPickerGUID() == Source->GetGUID())
        {
            ...
            message = GetNeoString(LANG_BG_WS_DROPPED_AF);
            type = CHAT_MSG_BG_SYSTEM_ALLIANCE;
...
void BattleGroundWS::EventPlayerClickedOnFlag(Player *Source, GameObject* target_obj)
{
    ...
    {
        message = GetNeoString(LANG_BG_WS_PICKEDUP_AF);
        type = CHAT_MSG_BG_SYSTEM_HORDE;
    ...
    {
        message = GetNeoString(LANG_BG_WS_PICKEDUP_HF);
        type = CHAT_MSG_BG_SYSTEM_ALLIANCE;
    ...
    if (this->GetFlagState(ALLIANCE) == BG_WS_FLAG_STATE_ON_GROUND && Source->IsWithinDistInMap(target_obj, 10) && target_obj->GetGOInfo()->id == BG_OBJECT_A_FLAG_GROUND_WS_ENTRY)
    {
        if (Source->GetTeam() == ALLIANCE)
        {
            message = GetNeoString(LANG_BG_WS_RETURNED_AF);
            type = CHAT_MSG_BG_SYSTEM_ALLIANCE;
    ...
    if (this->GetFlagState(HORDE) == BG_WS_FLAG_STATE_ON_GROUND && Source->IsWithinDistInMap(target_obj, 10) && target_obj->GetGOInfo()->id == BG_OBJECT_H_FLAG_GROUND_WS_ENTRY)
    {
        ...
        {
            message = GetNeoString(LANG_BG_WS_RETURNED_HF);
            type = CHAT_MSG_BG_SYSTEM_HORDE;
