...
#ifndef NEO_FORMULAS_H
#define NEO_FORMULAS_H
...
namespace Neo
{
    ...
    namespace Honor
    {
        ...
        inline int32 CalculeStanding(Player *plr)
        {
            ...
            QueryResult_AutoPtr result = CharacterDatabase.PQuery("SELECT count(*) as cnt FROM `characters` WHERE `honor_rating` >= '%f'", m_rating );
            if(result)
            {
    ...
    namespace XP
    {
        ...
        inline uint32 Gain(Player *pl, Unit *u)
        {
            ...
            {
			case 1:
				xp_gain *= sWorld.getRate(RATE_XP_KILL_HUMAN);
				break;
			case 2:
				xp_gain *= sWorld.getRate(RATE_XP_KILL_ORC);
				break;
			case 3:
				xp_gain *= sWorld.getRate(RATE_XP_KILL_DWARF);
				break;
			case 4:
				xp_gain *= sWorld.getRate(RATE_XP_KILL_NELF);
				break;
			case 5:
				xp_gain *= sWorld.getRate(RATE_XP_KILL_UNDEAD);
				break;
			case 6:
				xp_gain *= sWorld.getRate(RATE_XP_KILL_TAUREN);
				break;
			case 7:
				xp_gain *= sWorld.getRate(RATE_XP_KILL_GNOME);
				break;
			case 8:
				xp_gain *= sWorld.getRate(RATE_XP_KILL_TROLL);
				break;
			case 10:
				xp_gain *= sWorld.getRate(RATE_XP_KILL_BELF);
				break;
			case 11:
				xp_gain *= sWorld.getRate(RATE_XP_KILL_DRAE);
				break;
			default:
				xp_gain *= sWorld.getRate(RATE_XP_KILL_DEFAULT);
				break;
            }
			
			if(pl->GetSession()->GetRecruiterId() != 0 || pl->GetSession()->GetRecruitedId() != 0)
				xp_gain *= sWorld.getRate(RATE_XP_RAF_MULTIPLIER);			
