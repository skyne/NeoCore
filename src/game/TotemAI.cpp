...
TotemAI::UpdateAI(const uint32 /*diff*/)
{
    ...
    {
        CellPair p(Neo::ComputeCellPair(i_totem.GetPositionX(),i_totem.GetPositionY()));
        Cell cell(p);
        cell.data.Part.reserved = ALL_DISTRICT;

        victim = NULL;

        Neo::NearestAttackableUnitInObjectRangeCheck u_check(&i_totem, &i_totem, max_range);
        Neo::UnitLastSearcher<Neo::NearestAttackableUnitInObjectRangeCheck> checker(victim, u_check);

        TypeContainerVisitor<Neo::UnitLastSearcher<Neo::NearestAttackableUnitInObjectRangeCheck>, GridTypeMapContainer > grid_object_checker(checker);
        TypeContainerVisitor<Neo::UnitLastSearcher<Neo::NearestAttackableUnitInObjectRangeCheck>, WorldTypeMapContainer > world_object_checker(checker);
