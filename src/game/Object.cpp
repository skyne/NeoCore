    ...
    *data << (uint8)flags;                                  // update flags
    ...
    if (flags & UPDATEFLAG_LIVING)
    {
        ...
        if (flags2 & MOVEMENTFLAG_ONTRANSPORT)
        {
            ...
            //Neo currently not have support for other than player on transport
        }
...
bool WorldObject::IsWithinLOS(const float ox, const float oy, const float oz/*, Unit *targeter, Unit *target*/) const
{	
	/*
	VMAP::WorldModel Model = VMAP::WorldModel::WorldModel();		
	float targetheight   = Model.GetHeight(target->GetDisplayId());	  
	float targeterheight = Model.GetHeight(targeter->GetDisplayId());
	*/
    float x,y,z;
    GetPosition(x,y,z);
	VMAP::IVMapManager *vMapManager = VMAP::VMapFactory::createOrGetVMapManager();
//	vMapManager->getHeight();
