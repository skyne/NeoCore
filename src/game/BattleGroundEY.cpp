...
//            SpawnBGCreature(EY_SPIRIT_MAIN_HORDE, RESPAWN_IMMEDIATELY);
        ...
        {
            m_Events |= 0x04;
            SendMessageToAll(GetNeoString(LANG_BG_EY_ONE_MINUTE));
        }
        // After 1,5 minute, warning is signalled
        else if (GetStartDelayTime() <= START_DELAY2 && !(m_Events & 0x08))
        {
            m_Events |= 0x08;
            SendMessageToAll(GetNeoString(LANG_BG_EY_HALF_MINUTE));
        }
        ...
        else if (GetStartDelayTime() < 0 && !(m_Events & 0x10))
        {
            ...
            SendMessageToAll(GetNeoString(LANG_BG_EY_BEGIN));
...
void BattleGroundEY::RespawnFlag(bool send_message)
{
    ...
    if (send_message)
    {
        SendMessageToAll(GetNeoString(LANG_BG_EY_RESETED_FLAG));
...
void BattleGroundEY::EventPlayerDroppedFlag(Player *Source)
{
    ...
    {
        message = GetNeoString(LANG_BG_EY_DROPPED_FLAG);
...
void BattleGroundEY::EventPlayerClickedOnFlag(Player *Source, GameObject* target_obj)
{
    ...
    const char *message;
    uint8 type = 0;
    message = GetNeoString(LANG_BG_EY_HAS_TAKEN_FLAG);
...
void BattleGroundEY::EventTeamLostPoint(Player *Source, uint32 Point)
{
    ...
    if (Team == ALLIANCE)
    {
        ...
        message = GetNeoString(m_LoosingPointTypes[Point].MessageIdAlliance);
    ...
    else
    {
        ...
        message = GetNeoString(m_LoosingPointTypes[Point].MessageIdHorde);
...
void BattleGroundEY::EventTeamCapturedPoint(Player *Source, uint32 Point)
{
    ...
    if (Team == ALLIANCE)
    {
        m_TeamPointsCount[BG_TEAM_ALLIANCE]++;
        type = CHAT_MSG_BG_SYSTEM_ALLIANCE;
        message = GetNeoString(m_CapturingPointTypes[Point].MessageIdAlliance);
    ...
    else
    {
        m_TeamPointsCount[BG_TEAM_HORDE]++;
        type = CHAT_MSG_BG_SYSTEM_HORDE;
        message = GetNeoString(m_CapturingPointTypes[Point].MessageIdHorde);
...
void BattleGroundEY::EventPlayerCapturedFlag(Player *Source, uint32 BgObjectType)
{
    ...
    if (Source->GetTeam() == ALLIANCE)
    {
        ...
        message = GetNeoString(LANG_BG_EY_CAPTURED_FLAG_A);
        type = CHAT_MSG_BG_SYSTEM_ALLIANCE;
    }
    else
    {
        ...
        message = GetNeoString(LANG_BG_EY_CAPTURED_FLAG_H);
        type = CHAT_MSG_BG_SYSTEM_HORDE;
    }
