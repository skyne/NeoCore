...
// Neo string ranges
#define MIN_NEO_STRING_ID           1                    // 'mangos_string'
#define MAX_NEO_STRING_ID           2000000000
#define MIN_DB_SCRIPT_STRING_ID        MAX_NEO_STRING_ID // 'db_script_string'
#define MAX_DB_SCRIPT_STRING_ID        2000010000
#define MIN_CREATURE_AI_TEXT_STRING_ID (-1)                 // 'creature_ai_texts'
#define MAX_CREATURE_AI_TEXT_STRING_ID (-1000000)

struct NeoStringLocale
{
...
typedef UNORDERED_MAP<uint32,NeoStringLocale> NeoStringLocaleMap;
...
struct NEO_DLL_SPEC LanguageDesc
{
    Language lang_id;
    uint32   spell_id;
    uint32   skill_id;
...
NEO_DLL_SPEC LanguageDesc const* GetLanguageDescByID(uint32 lang);
...
class ObjectMgr
{
    ...
    public:
        ...
        bool LoadNeoStrings(DatabaseType& db, char const* table, int32 min_value, int32 max_value);
        bool LoadNeoStrings() { return LoadNeoStrings(WorldDatabase,"neo_string",MIN_NEO_STRING_ID,MAX_NEO_STRING_ID); }
    void LoadDbScriptStrings();
        ...
        NeoStringLocale const* GetNeoStringLocale(int32 entry) const
        {
            NeoStringLocaleMap::const_iterator itr = mNeoStringLocaleMap.find(entry);
            if (itr==mNeoStringLocaleMap.end()) return NULL;
            return &itr->second;
        }
        const char *GetNeoString(int32 entry, int locale_idx) const;
        const char *GetNeoStringForDBCLocale(int32 entry) const { return GetNeoString(entry,DBCLocaleIndex); }
    int32 GetDBCLocaleIndex() const { return DBCLocaleIndex; }
        ...
        void AddVendorItem(uint32 entry,uint32 item, int32 maxcount, uint32 incrtime, int32 ExtendedCost, uint8 minlvl, uint8 maxlvl, bool savetodb = true); // for event
    ...
    private:
        ...
        NeoStringLocaleMap mNeoStringLocaleMap;
...
#define objmgr Neo::Singleton<ObjectMgr>::Instance()

// scripting access functions
NEO_DLL_SPEC bool LoadNeoStrings(DatabaseType& db, char const* table,int32 start_value = MAX_CREATURE_AI_TEXT_STRING_ID, int32 end_value = std::numeric_limits<int32>::min());
NEO_DLL_SPEC uint32 GetAreaTriggerScriptId(uint32 trigger_id);
NEO_DLL_SPEC uint32 GetScriptId(const char *name);
NEO_DLL_SPEC ObjectMgr::ScriptNameMap& GetScriptNames();
NEO_DLL_SPEC GameObjectInfo const *GetGameObjectInfo(uint32 id);
NEO_DLL_SPEC CreatureInfo const *GetCreatureInfo(uint32 id);
NEO_DLL_SPEC CreatureInfo const* GetCreatureTemplateStore(uint32 entry);
NEO_DLL_SPEC Quest const* GetQuestTemplateStore(uint32 entry);
