...
void BattleGround::EndBattleGround(uint32 winner)
{
    ...
    {
        if (isBattleGround())
            winmsg = GetNeoString(LANG_BG_A_WINS);
        else
            winmsg = GetNeoString(LANG_ARENA_GOLD_WINS);
    ...
    {
        if (isBattleGround())
            winmsg = GetNeoString(LANG_BG_H_WINS);
        else
            winmsg = GetNeoString(LANG_ARENA_GREEN_WINS);
...
void BattleGround::SendRewardMarkByMail(Player *plr,uint32 mark, uint32 count)
{
    ...
    if (Item* markItem = Item::CreateItem(mark,count,plr))
    {
        ...
        // text
        std::string textFormat = plr->GetSession()->GetNeoString(LANG_BG_MARK_BY_MAIL);
        char textBuf[300];
...
{
    char const* text = GetNeoString(entry);
    WorldPacket data;
    ChatHandler::FillMessageData(&data, NULL, CHAT_MSG_BG_SYSTEM_NEUTRAL, LANG_UNIVERSAL, NULL, 0, text, NULL);
    SendPacketToAll(&data);
}
