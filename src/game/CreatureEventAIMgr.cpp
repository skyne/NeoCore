...
void CreatureEventAIMgr::LoadCreatureEventAI_Texts(bool check_entry_use)
{
    ...
    // Load EventAI Text
    objmgr.LoadNeoStrings(WorldDatabase,"creature_ai_texts",MIN_CREATURE_AI_TEXT_STRING_ID,MAX_CREATURE_AI_TEXT_STRING_ID);
    ...
    if (result)
    {
        ...
        do
        {
            ...
            if (!objmgr.GetNeoStringLocale(i))
            {
...
void CreatureEventAIMgr::LoadCreatureEventAI_Summons(bool check_entry_use)
{
    ...
    if (result)
    {
        ...
        do
        {
            ...
            if(!Neo::IsValidMapCoord(temp.position_x,temp.position_y,temp.position_z,temp.orientation))
            {
            ...
            for (uint32 j = 0; j < MAX_ACTIONS; j++)
            {
                ...
                switch (action.type)
                {
                    ...
                    case ACTION_T_QUEST_EVENT:
                        ...
                        {
                            if (!qid->HasFlag(QUEST_NEO_FLAGS_EXPLORATION_OR_EVENT))
