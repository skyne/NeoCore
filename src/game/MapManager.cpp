...
#define CLASS_LOCK Neo::ClassLevelLockable<MapManager, ACE_Thread_Mutex>
    ...
    if (entry->map_type == MAP_INSTANCE || entry->map_type == MAP_RAID)
    {
        if (entry->map_type == MAP_RAID)
        {
            ...
            if (!player->isGameMaster() && !sWorld.getConfig(CONFIG_INSTANCE_IGNORE_RAID))
            {
                ...
                if (!group || !group->isRaidGroup())
                {
                    ...
                    player->GetSession()->SendAreaTriggerMessage(player->GetSession()->GetNeoString(810), mapName);
        ...
        if (!player->isAlive())
        {
            if (Corpse *corpse = player->GetCorpse())
            {
                ...
                if (!instance_map)
                {
                    player->GetSession()->SendAreaTriggerMessage(player->GetSession()->GetNeoString(811), mapName);
