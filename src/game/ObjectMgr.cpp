...
void ObjectMgr::LoadCreatureLocales()
{
    ...
    QueryResult_AutoPtr result = WorldDatabase.Query("SELECT entry,name_loc1,subname_loc1,name_loc2,subname_loc2,name_loc3,subname_loc3,name_loc4,subname_loc4,name_loc5,subname_loc5,name_loc6,subname_loc6,name_loc7,subname_loc7,name_loc8,subname_loc8,name_loc9,subname_loc9 FROM locales_creature");
...
void ObjectMgr::LoadNpcOptionLocales()
{
    ...
    QueryResult_AutoPtr result = WorldDatabase.Query("SELECT entry,"
        ...
        "option_text_loc7,box_text_loc7,option_text_loc8,box_text_loc8,option_text_loc9,box_text_loc9 "
...
void ObjectMgr::AddCreatureToGrid(uint32 guid, CreatureData const* data)
{
    ...
    for (uint8 i = 0; mask != 0; i++, mask >>= 1)
    {
        if (mask & 1)
        {
            CellPair cell_pair = Neo::ComputeCellPair(data->posX, data->posY);
...
void ObjectMgr::LoadItemLocales()
{
    ...
    QueryResult_AutoPtr result = WorldDatabase.Query("SELECT entry,name_loc1,description_loc1,name_loc2,description_loc2,name_loc3,description_loc3,name_loc4,description_loc4,name_loc5,description_loc5,name_loc6,description_loc6,name_loc7,description_loc7,name_loc8,description_loc8,name_loc9,description_loc9 FROM locales_item");
        ...
        do
        {
            ...
            if (current_level > sWorld.getConfig(CONFIG_MAX_PLAYER_LEVEL))
            {
                if (current_level > STRONG_MAX_LEVEL)        // hardcoded level maximum
                    ...
                    sLog.outDetail("Unused (> MaxPlayerLevel in Neod.conf) level %u in pet_levelstats table, ignoring.",current_level);
                continue;
            }
        ...
        do
        {
            ...
            if (current_level > sWorld.getConfig(CONFIG_MAX_PLAYER_LEVEL))
            {
                if (current_level > STRONG_MAX_LEVEL)        // hardcoded level maximum
                    ...
                    sLog.outDetail("Unused (> MaxPlayerLevel in Neod.conf) level %u in player_classlevelstats table, ignoring.",current_level);
                continue;
            }
        ...
        do
        {
            ...
            if (current_level > sWorld.getConfig(CONFIG_MAX_PLAYER_LEVEL))
            {
                if (current_level > STRONG_MAX_LEVEL)        // hardcoded level maximum
                    ...
                    sLog.outDetail("Unused (> MaxPlayerLevel in Neod.conf) level %u in player_levelstats table, ignoring.",current_level);
                continue;
            }
...
void ObjectMgr::LoadQuests()
{
    ...
    for (QuestMap::iterator iter = mQuestTemplates.begin(); iter != mQuestTemplates.end(); ++iter)
    {
        ...
        if (qinfo->QuestFlags & ~QUEST_NEO_FLAGS_DB_ALLOWED)
        {
            sLog.outErrorDb("Quest %u has SpecialFlags = %u > max allowed value. Correct SpecialFlags to value <= %u",
                qinfo->GetQuestId(),qinfo->QuestFlags,QUEST_NEO_FLAGS_DB_ALLOWED >> 16);
            qinfo->QuestFlags &= QUEST_NEO_FLAGS_DB_ALLOWED;
        }

        if (qinfo->QuestFlags & QUEST_FLAGS_DAILY)
        {
            if (!(qinfo->QuestFlags & QUEST_NEO_FLAGS_REPEATABLE))
            {
                sLog.outErrorDb("Daily Quest %u not marked as repeatable in SpecialFlags, added.",qinfo->GetQuestId());
                qinfo->QuestFlags |= QUEST_NEO_FLAGS_REPEATABLE;
            }
        }
        ...
        for (int j = 0; j < QUEST_OBJECTIVES_COUNT; ++j )
        {
            ...
            if (id)
            {
                ...
                qinfo->SetFlag(QUEST_NEO_FLAGS_DELIVER);
        ...
        for (int j = 0; j < QUEST_OBJECTIVES_COUNT; ++j )
        {
            ...
            if (id)
            {
                ...
                if (!qinfo->ReqCreatureOrGOId[j])
                {
                    ...
                    if (found)
                    {
                        if (!qinfo->HasFlag(QUEST_NEO_FLAGS_EXPLORATION_OR_EVENT))
                        {
                            sLog.outErrorDb("Spell (id: %u) have SPELL_EFFECT_QUEST_COMPLETE or SPELL_EFFECT_SEND_EVENT for quest %u and ReqCreatureOrGOId%d = 0, but quest not have flag QUEST_NEO_FLAGS_EXPLORATION_OR_EVENT. Quest flags or ReqCreatureOrGOId%d must be fixed, quest modified to enable objective.",spellInfo->Id,qinfo->QuestId,j+1,j+1);

                            // this will prevent quest completing without objective
                            const_cast<Quest*>(qinfo)->SetFlag(QUEST_NEO_FLAGS_EXPLORATION_OR_EVENT);
                        }
                    }
                    else
                    {
        ...
        for (int j = 0; j < QUEST_OBJECTIVES_COUNT; ++j )
        {
            ...
            if (id)
            {
                ...
                qinfo->SetFlag(QUEST_NEO_FLAGS_KILL_OR_CAST | QUEST_NEO_FLAGS_SPEAKTO);
        ...
        if (qinfo->LimitTime)
            qinfo->SetFlag(QUEST_NEO_FLAGS_TIMED);
    }

    // check QUEST_NEO_FLAGS_EXPLORATION_OR_EVENT for spell with SPELL_EFFECT_QUEST_COMPLETE
    for (uint32 i = 0; i < sSpellStore.GetNumRows(); ++i)
    {
        ...
        for (int j = 0; j < 3; ++j)
        {
            ...
            if (!quest->HasFlag(QUEST_NEO_FLAGS_EXPLORATION_OR_EVENT))
            {
                sLog.outErrorDb("Spell (id: %u) have SPELL_EFFECT_QUEST_COMPLETE for quest %u , but quest not have flag QUEST_NEO_FLAGS_EXPLORATION_OR_EVENT. Quest flags must be fixed, quest modified to enable objective.",spellInfo->Id,quest_id);

                // this will prevent quest completing without objective
                const_cast<Quest*>(quest)->SetFlag(QUEST_NEO_FLAGS_EXPLORATION_OR_EVENT);
            }
        }
    }
    ...
    do
    {
        ...
        switch(tmp.command)
        {
            ...
            case SCRIPT_COMMAND_TELEPORT_TO:
            {
                ...
                if (!Neo::IsValidMapCoord(tmp.x,tmp.y,tmp.z,tmp.o))
                {
            ...
            case SCRIPT_COMMAND_QUEST_EXPLORED:
            {
                ...
                if (!quest->HasFlag(QUEST_NEO_FLAGS_EXPLORATION_OR_EVENT))
                {
                    sLog.outErrorDb("Table %s has quest (ID: %u) in SCRIPT_COMMAND_QUEST_EXPLORED in datalong for script id %u, but quest not have flag QUEST_NEO_FLAGS_EXPLORATION_OR_EVENT in quest flags. Script command or quest flags wrong. Quest modified to require objective.",tablename,tmp.datalong,tmp.id);
...
void ObjectMgr::LoadPageTextLocales()
{
    ...
    QueryResult_AutoPtr result = WorldDatabase.Query("SELECT entry,text_loc1,text_loc2,text_loc3,text_loc4,text_loc5,text_loc6,text_loc7,text_loc8,text_loc9 FROM locales_page_text");
...
void ObjectMgr::LoadQuestAreaTriggers()
{
    ...
    do
    {
        ...
        if (!quest->HasFlag(QUEST_NEO_FLAGS_EXPLORATION_OR_EVENT))
        {
            sLog.outErrorDb("Table areatrigger_involvedrelation has record (id: %u) for not quest %u, but quest not have flag QUEST_NEO_FLAGS_EXPLORATION_OR_EVENT. Trigger or quest flags must be fixed, quest modified to require objective.",trigger_ID,quest_ID);
...
void ObjectMgr::LoadGameObjectLocales()
{
    ...
    QueryResult_AutoPtr result = WorldDatabase.Query("SELECT entry,"
        "name_loc1,name_loc2,name_loc3,name_loc4,name_loc5,name_loc6,name_loc7,name_loc8,name_loc9,"
        "castbarcaption_loc1,castbarcaption_loc2,castbarcaption_loc3,castbarcaption_loc4,"
        "castbarcaption_loc5,castbarcaption_loc6,castbarcaption_loc7,castbarcaption_loc8,castbarcaption_loc9 FROM locales_gameobject");
...
bool ObjectMgr::LoadNeoStrings(DatabaseType& db, char const* table, int32 min_value, int32 max_value)
{
    ...
    for (NeoStringLocaleMap::iterator itr = mNeoStringLocaleMap.begin(); itr != mNeoStringLocaleMap.end();)
    {
        if (itr->first >= start_value && itr->first < end_value)
            mNeoStringLocaleMap.erase(itr++);
        else
             ++itr;
    }

    QueryResult_AutoPtr result = db.PQuery("SELECT entry,content_default,content_loc1,content_loc2,content_loc3,content_loc4,content_loc5,content_loc6,content_loc7,content_loc8,content_loc9 FROM %s",table);
    ...
    if (!result)
    {
        ...
        sLog.outString("");
        if (min_value == MIN_NEO_STRING_ID)              // error only in case internal strings
            sLog.outErrorDb(">> Loaded 0 Neo strings. DB table %s is empty. Cannot continue.",table);
        else
    ...
    do
    {
        ...
        NeoStringLocale& data = mNeoStringLocaleMap[entry];
    ...
    sLog.outString("");
    if (min_value == MIN_NEO_STRING_ID)
        sLog.outString(">> Loaded %u Neo strings from table %s", count,table);
    else
...
const char *ObjectMgr::GetNeoString(int32 entry, int locale_idx) const
{
    // locale_idx==-1 -> default, locale_idx >= 0 in to idx+1
    // Content[0] always exist if exist NeoStringLocale
    if (NeoStringLocale const *msl = GetNeoStringLocale(entry))
    {
    ...
    if (entry > 0)
        sLog.outErrorDb("Entry %i not found in neo_string table.",entry);
    else
        sLog.outErrorDb("Neo string entry %i not found in DB.",entry);
    return "<error>";
}
...
void ObjectMgr::LoadVendors()
{
   ...
   QueryResult_AutoPtr result = WorldDatabase.Query("SELECT entry, item, maxcount, incrtime, ExtendedCost, minlvl, maxlvl FROM npc_vendor ORDER BY entry");
   if (!result )
    {
        barGoLink bar(1);
    ...
    do
    {
		...
		uint8  minlvl      	= fields[5].GetUInt8();
		uint8  maxlvl		= fields[6].GetUInt8();
...
void ObjectMgr::AddVendorItem(uint32 entry,uint32 item, int32 maxcount, uint32 incrtime, int32 extendedcost, uint8 minlvl, uint8 maxlvl, bool savetodb)
{
    VendorItemData& vList = m_mCacheVendorItemMap[entry];
    vList.AddItem(item,maxcount,incrtime,extendedcost,minlvl,maxlvl);

    if (savetodb) WorldDatabase.PExecuteLog("INSERT INTO npc_vendor (entry,item,maxcount,incrtime,extendedcost,minlvl,maxlvl) VALUES('%u','%u','%u','%u','%i','%u','%u')",entry, item, maxcount,incrtime,extendedcost,minlvl,maxlvl);
}
...
void ObjectMgr::CheckScripts(ScriptMapMap const& scripts,std::set<int32>& ids)
{
    for (ScriptMapMap::const_iterator itrMM = scripts.begin(); itrMM != scripts.end(); ++itrMM)
    {
        for (ScriptMap::const_iterator itrM = itrMM->second.begin(); itrM != itrMM->second.end(); ++itrM)
        {
            ...
            {
                if (!GetNeoStringLocale (itrM->second.dataint))
...
{
    LoadNeoStrings(WorldDatabase,"db_script_string",MIN_DB_SCRIPT_STRING_ID,MAX_DB_SCRIPT_STRING_ID);

    std::set<int32> ids;

    for (int32 i = MIN_DB_SCRIPT_STRING_ID; i < MAX_DB_SCRIPT_STRING_ID; ++i)
        if (GetNeoStringLocale(i))
            ids.insert(i);
...
bool LoadNeoStrings(DatabaseType& db, char const* table,int32 start_value, int32 end_value)
{
    ...
    return objmgr.LoadNeoStrings(db,table,start_value,end_value);
}

uint32 NEO_DLL_SPEC GetScriptId(const char *name)
{
