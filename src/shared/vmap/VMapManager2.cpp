...
namespace VMAP
{
    ...
    void VMapManager2::preventMapsFromBeingUsed(const char* pMapIdString)
    {
        ...
        if (pMapIdString != NULL)
        {
            ...
            while (std::getline(map_ss, map_str, ','))
            {
                ...
                if (map_num >= 0)
                {
					sLog.outDetail("ingoring Map %i for VMaps",map_num);                    
    ...
    */
    ...
    WorldModel* VMapManager2::acquireModelInstance(const std::string &basepath, const std::string &filename)
    {
        ...
        if (model == iLoadedModelFiles.end())
        {
            ...
            if (!worldmodel->readFile(basepath + filename + ".vmo"))
            {
				...
				sLog.outDetail("VMapManager2: could not load '%s%s .vmo'!",basepath.c_str(),filename.c_str());
                delete worldmodel;
                return NULL;
            }
            sLog.outDetail("VMapManager2: loading file '%s%s'",basepath.c_str(),filename.c_str());
    ...
    void VMapManager2::releaseModelInstance(const std::string &filename)
    {
        ...
        if (model == iLoadedModelFiles.end())
        {
            sLog.outDetail("VMapManager2: trying to unload non-loaded file '%s'",filename.c_str());
            return;
        }
        if( model->second.decRefCount() == 0)
        {
            sLog.outDetail("VMapManager2: unloading file '%s'",filename.c_str());
